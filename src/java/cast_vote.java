...
public class cast_vote extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
           ...
           String data=user+"@"+party;
            System.out.println(data);
           File file = new File(path_info.pypath+"data.txt"); 
                    FileOutputStream fout=new FileOutputStream(file);
                    fout.write(data.getBytes());
                    fout.close();
                    File file1 = new File(path_info.pypath+"task.txt"); 
                    FileOutputStream fout1=new FileOutputStream(file1);
                    fout1.write("upload".getBytes());
                    fout1.close();
//            DBQuery db=new DBQuery();
//            try {
//                db.get_voting_count(party, user);
//                out.print("Updated");
//            } catch (SQLException ex) {
//                Logger.getLogger(cast_vote.class.getName()).log(Level.SEVERE, null, ex);
//            }
